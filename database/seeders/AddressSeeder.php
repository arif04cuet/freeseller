    ...
    public function run(): void
    {
        ...
        foreach ($divisions as $division) {
            Address::updateOrcreate(
                ['id' => (int) $division['id']],
                [
                    'name' => $division['bn_name'] . ' বিভাগ',
                    'type' => AddressType::Division->value,
                    'parent_id' => null,
                ]
            );
        }
        ...
        $tangail = Address::whereName('টাঙ্গাইল জেলা')->first();
        $kalihati =  Address::updateOrcreate(
            ['name' => 'কালিহাতী'],
            [
                'type' => AddressType::Upazila->value,
                'parent_id' => $tangail->id,
            ]
        );

        $bolla = Address::updateOrcreate(
            ['name' => 'বল্লা'],
            [
                'type' => AddressType::Union->value,
                'parent_id' => $kalihati->id,
            ]
        );

        $rampur = Address::updateOrcreate(
            ['name' => 'রামপুর ভাসানী মার্কেট'],
            [
                'type' => AddressType::Hub->value,
                'parent_id' => $bolla->id,
            ]
        );
    }
